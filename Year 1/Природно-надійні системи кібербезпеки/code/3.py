...
encoder_srn_outputs, encoder_state_h = encoder(encoder_inputs)
...
decoder_outputs = decoder_dense(decoder_srn_outputs)
...
encoder_model = tensorflow.keras.Model(encoder_inputs, encoder_state_h)

decoder_inputs = model.input[1]
decoder_input_state_h = tensorflow.keras.Input(shape=(latent_dim,))
decoder_srn = model.layers[3]
decoder_srn_outputs, decoder_state_h = decoder_srn(decoder_inputs, initial_state=decoder_input_state_h)

decoder_dense = model.layers[4]
decoder_outputs = decoder_dense(decoder_srn_outputs)
decoder_model = tensorflow.keras.Model(
    [decoder_inputs] + [decoder_input_state_h], [decoder_outputs] + [decoder_state_h]
)
...
for seq_index in range(20):
    ...
    h = encoder_model.predict(input_seq, verbose=0)

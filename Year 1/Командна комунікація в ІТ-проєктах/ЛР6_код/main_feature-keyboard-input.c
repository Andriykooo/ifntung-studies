 ...
 * Гілка feature/keyboard-input (Бойчук А. А.):
 * розмір масиву та значення елементів вводяться з клавіатури.
 */
#include <stdio.h>
...
int main(void)
{
    ...
    printf("Введіть %d елементів масиву через пробіл: ", n);
    for (int i = 0; i < n; i++) {
        if (scanf("%lf", &a[i]) != 1) {
            printf("Некоректне значення елемента %d\n", i + 1);
            return 1;
        }
    }
    printf("\n");

 ...
 * Гілка feature/formatted-output: вивід елементів масиву в рядок
 * з використанням форматованого виводу (фіксована ширина поля).
 */
#include <stdio.h>
...
void print_array(const char *title, const double a[], int n)
{
    printf("%-24s|", title);
    for (int i = 0; i < n; i++)
        printf("%7.2f", a[i]);          /* ширина 7, 2 знаки після коми */
    printf(" |\n");
}
...
void transform(const double a[], double c[], int n)
{
    ...
    printf("Середнє від'ємних = %.2f, середнє додатних = %.2f\n", mean_neg, mean_pos);
